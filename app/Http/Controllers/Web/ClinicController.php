...
class ClinicController extends Controller
{
	...
	if ($validator->fails()) {
		...
		if($request->filter_name== 'count'){
			...
			$appointments = Appointment::where('clinic_patient_id',$request->patient_id)->with('clinic_patient')->with('diagnosis')->with('voucher')->with('voucher.counting_unit')->with('voucher.counting_unit.item')->orderBy('id','desc')->take($request->count_app)->get();
		}
			...
			$appointments = Appointment::where('clinic_patient_id',$request->patient_id)->whereBetween('date',[$request->from_date,$request->to_date])->with('clinic_patient')->with('diagnosis')->with('voucher')->with('voucher.counting_unit')->with('voucher.counting_unit.item')->orderBy('id','desc')->get();
		}
		else{
		$appointments = Appointment::where('clinic_patient_id',$request->patient_id)->with('clinic_patient')->with('diagnosis')->with('voucher')->with('voucher.counting_unit')->with('voucher.counting_unit.item')->orderBy('id','desc')->get();
		}
